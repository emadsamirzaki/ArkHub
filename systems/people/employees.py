...
def _employee_table() -> None:
    ...
    # Column headers
    h1, h2, h3, h4, h5, h6 = st.columns([3, 3, 2, 1.5, 1, 1])
    h1.markdown("**Name**")
    h2.markdown("**Email**")
    h3.markdown("**Role**")
    h4.markdown("**Status**")
    ...
    for emp in employees:
        ...
        if st.session_state.get(f"emp_edit_{eid}"):
            ...
            with st.container(border=True):
                ec1, ec2, ec3, ec4 = st.columns([3, 3, 2, 1.5])
                ...
                new_status = ec4.selectbox("Status",  ["Active", "Inactive"],
            ...
            # ── Display row ───────────────────────────────────────────────────
            c1, c2, c3, c4, c5, c6 = st.columns([3, 3, 2, 1.5, 1, 1])
            badge = (
            ...
            c3.markdown(f'<div class="emp-row-role">{emp["role"]}</div>',   unsafe_allow_html=True)
            c4.markdown(badge, unsafe_allow_html=True)

            if c5.button("Edit", key=f"ebtn_{eid}", use_container_width=True):
                st.session_state[f"emp_edit_{eid}"] = True
                st.rerun()

            if c6.button("Delete", key=f"ddel_{eid}", use_container_width=True):
